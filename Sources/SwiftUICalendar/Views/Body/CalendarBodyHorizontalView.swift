...
struct CalendarBodyHorizontalView: View {
                ...
                DragGesture()
                    ...
                    .onEnded { value in
                        ...
                        let resolvedTranslation = HorizontalMonthSwipeResolver.resolvedTranslation(
                            ...
                            predictedEndTranslation: value.predictedEndTranslation.width * layoutDirectionMultiplier,
                            limit: layoutWidth
                        )
    ...
    private var rowCountForHeight: Int {
        ...
        case .hugContent:
            let current = currentViewModel.rowCount(month: currentViewModel.currentMonth, year: currentViewModel.currentYear)
            let previous = previousViewModel.rowCount(month: previousViewModel.currentMonth, year: previousViewModel.currentYear)
            let next = nextViewModel.rowCount(month: nextViewModel.currentMonth, year: nextViewModel.currentYear)
...
private struct HorizontalMonthHeightPreferenceKey: PreferenceKey {
    ...
    static func reduce(value: inout [HorizontalMonthPosition: CGFloat], nextValue: () -> [HorizontalMonthPosition: CGFloat]) {
